    ...
    for file in media:
        ...
        if type == "images":
            ...
            await context.bot.send_photo(
                chat_id=chat_id, photo=response.content, caption=caption
            )

        elif type == "audios":
            response = requests.get(url)
            await context.bot.send_audi(
                chat_id=chat_id, audio=response.content, caption=caption
            )

        elif type == "videos":
            response = requests.get(url)
            await context.bot.send_video(
                chat_id=chat_id, video=response.content, caption=caption
            )

        elif type == "documents":
            response = requests.get(url)
            await context.bot.send_document(
                chat_id=chat_id, document=response.content, caption=caption
            )

        elif type == "stickers":
            response = requests.get(url)
            await context.bot.send_stickers(
                chat_id=chat_id, sticker=response.content, caption=caption
            )
